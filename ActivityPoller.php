...
Class WorkflowActivityPoller
{
	...
	public function poll_for_activities()
	{
		...
		if (!$result)
            {
                log_out("ERROR", basename(__FILE__), "Activity result is empty ! Activity failed ...");
                $activity["object"]->activity_failed($activityTask, self::EMPTY_RESULT, "Empty output result !");
                return true;
            }
        
        log_out("INFO", basename(__FILE__),"Activity completed !");
